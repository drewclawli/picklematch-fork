...
export const PlayerSetup = ({ onComplete, initialPlayers = [], initialTeammatePairs = [] }: PlayerSetupProps) => {
  ...
  const removePlayer = (index: number) => {
    ...
    setTeammatePairs(teammatePairs.filter(pair => 
      pair.player1 !== playerToRemove && pair.player2 !== playerToRemove
    ));
      ...
      {teammatePairs.length > 0 && (
        <div className="pt-4 border-t space-y-2">
          ...
          {teammatePairs.map((pair, idx) => (
            <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-accent/10 border border-accent/20">
